...
def worker(url, num_requests, duration):
    ...
    while time.time() < end_time:
        ...
        try:
            response = requests.post(url, data=data, timeout=5)  # Adding a timeout to prevent hangs
...
def benchmark(url, num_threads, num_requests_per_thread, duration):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = [executor.submit(worker, url, num_requests_per_thread, duration) for _ in range(num_threads)]
